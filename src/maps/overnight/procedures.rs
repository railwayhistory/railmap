...
use super::feature::markers::Statdot;
use super::feature::route::RouteContour;
...
*/
    ...
    // ```
    ("casing", &|_pos, _args, _scope, _features, _err| {
        /*
        let [class, trace] = args.into_positionals(err)?;
    ...
    // ```
    ("line_badge", &|_pos, _args, _scope, _features, _err| {
        /*
        let (properties, position, layout) = label_args(
    ...
    // ```
    ("marker", &|_pos, _args, _scope, _features, _err| {
        /*
...
*/
    ...
    //
    // ```text
    // route(class: symbol-set, path: trace)
    // ```
    ("route", &|pos, args, scope, features, err| {
        features.insert(
...
*/
    ...
    ("statdot", &|pos, args, scope, features, err| {
        let [class, position, left, right] = args.into_positionals(err)?;
        let class = Class::from_arg(class, err);
        let position = position.into_position(err);
        let left = left.into_distance(err);
        let right = right.into_distance(err);
        features.insert(
            Feature::Statdot(Statdot::new(
    ...
    // ```
    ("track", &|_pos, _args, _scope, _features, _err| {
        /*
        let [class, trace] = args.into_positionals(err)?;
        let class = TrackClass::from_arg(class, err)?;
        let layer_offset = class.class().layer_offset();
...
impl Procedure {
    ...
    pub fn eval(
        ...
        features: &mut FeatureSet<Overnight>,
        err: &mut eval::Error,
    ) -> Result<(), Failed> {
...
fn label_args(
    args: eval::ArgumentList<Overnight>,
    err: &mut eval::Error,
    base_properties: label::Properties
