...
use femtomap::world;
use femtomap::import::ast::{Pos, ShortString};
...
impl StandardMarker {
    pub fn new(
        ...
        let marker = match OLD_MARKERS.get(marker.as_str()) {
            Some(marker) => *marker,
            None => {
                err.add(marker_pos, "missing marker");
                return Err(Failed)
            }
        };
        Ok(StandardMarker { position, orientation, class, marker }.into())
    }
...
markers! {
    ...
    ("de.bk", "block") => (
        |canvas: &mut Group, u: Measures| {
            ...
            stroke_round(canvas)
            */
            chevron(canvas, 0.3 * u.sw() - 0.5 * u.sp(), 1.3 * u.main_track(), u.sh());
            canvas.apply_line_width(u.sp());
            canvas.stroke();
        },
        |canvas: &mut Group, u: Measures| {
    ...
    ("tunnel.mo") => (
        |canvas: &mut Group, u: Measures| {
            canvas.apply_line_width(u.sp());
            canvas.move_to(-0.5 * u.dt(), 0.);
            canvas.line_to(0.5 * u.dt(), 0.);
            canvas.stroke()
        }
    ),
    ("tunnel.dt") => (
...
fn stop(
    ...
    let hsp = 0.5 * u.main_track();
    canvas.move_to(-0.5 * u.sw() + hsp, 1.5 * u.main_track());
    canvas.line_to(-0.5 * u.sw() + hsp, u.sh() - 0.5 * u.sp());
    canvas.line_to(0.5 * u.sw() - hsp, u.sh() - 0.5 * u.sp());
    canvas.line_to(0.5 * u.sw() - hsp, 1.5 * u.main_track());
    canvas.close_path();
    canvas.apply_line_width(u.main_track());
    canvas.stroke();
    canvas.new_path();
...
fn stop_small(
    ...
    let y0 = 1.5 * u.main_skip() + 0.5 * u.main_track();
    let hsp = 0.4 * u.sp();
    canvas.move_to(-0.5 * u.sw() + hsp, y0);
    canvas.line_to(-0.5 * u.sw() + hsp, u.sh() - hsp);
    canvas.line_to(0.5 * u.sw() - hsp, u.sh() - hsp);
    canvas.line_to(0.5 * u.sw() - hsp, y0);
    canvas.close_path();
